...
let appState = {
    currentUser: null,
    userRole: 'guest', // 'admin', 'journalist', ou 'guest'
    editMode: false,
    hero: [],       
    news: [],       
    shows: [],      
...
function renderAll() {
    ...
    const renderGrid = (gridElement, dataArray, category, tableName) => {
        ...
        const canEditThisCategory = isAdmin || (role === 'journalist' && (category === 'hero' || category === 'news'));
        const hasDedicatedPage = (category === 'hero' || category === 'news');

        gridElement.innerHTML = dataArray.map((item) => `
            <div class="card ${!item.is_published ? 'draft-card' : ''} ${!hasDedicatedPage ? 'no-click-page' : ''}" 
                 ${hasDedicatedPage ? `onclick="openArticleView('${category}', '${item.id}')"` : ''}
                 style="${!hasDedicatedPage ? 'cursor: default;' : ''}">
...
function openEditorModal(category, id = null) {
    ...
    if (role === 'journalist' && (category === 'shows' || category === 'team')) {
        alert("🔒 Accès restreint : Seuls les administrateurs peuvent modifier les émissions et l'équipe.");
        return;
    }
...
function checkAdminRights(user) {
    ...
    const isStaff = (role === 'admin' || role === 'journalist');
    appState.editMode = isStaff;

    document.body.classList.toggle('admin-logged-in', isStaff);
    document.body.classList.toggle('edit-mode-active', isStaff);
}
...
/* ==========================================================================
   10. LECTEUR AUDIO & MÉTADONNÉES (STABLE & SANS PROXY EXTERNE)
   ========================================================================== */
