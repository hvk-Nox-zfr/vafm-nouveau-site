...
// Helper pour récupérer l'URL PocketBase de manière sécurisée
function getPbUrl() {
    return typeof POCKETBASE_URL !== 'undefined' ? POCKETBASE_URL : (window.POCKETBASE_URL || '');
}
...
async function openArticleView(category, id) {
    ...
    try {
        ...
        const response = await fetch(`${baseUrl}/api/collections/${collectionName}/records/${id}?expand=author,user,user_id`);
...
async function handleTogglePublishInStudio(collectionName, id, currentStatus, category) {
    ...
    try {
        ...
        const response = await fetch(`${baseUrl}/api/collections/${collectionName}/records/${id}`, {
            method: 'PATCH',
            headers: headers,
...
async function handleCanvaImageUpload(event) {
    ...
    try {
        ...
        const baseUrl = getPbUrl();
        const res = await fetch(`${baseUrl}/api/collections/article_images/records`, {
            method: 'POST',
            headers,
            body: fd
        });
        ...
        const finalUrl = (typeof getPocketBaseImageUrl === 'function')
            ...
            : `${baseUrl}/api/files/article_images/${record.id}/${record.image}?thumb=1000x0`;
